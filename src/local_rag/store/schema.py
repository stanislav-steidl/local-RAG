...
def build_schema(dimension: int) -> pa.Schema:
    """Describe the table layout for a given dense vector width.

    The width is a parameter rather than a constant because it belongs to
    whichever embedder is in use, and a table built for one model cannot hold
    another's vectors.
...
def to_record(chunk: Chunk, embedding: Embedding) -> dict[str, Any]:
    ...
    return {
        ...
        "extra_json": json.dumps(document.extra, sort_keys=True, default=str),
    }
